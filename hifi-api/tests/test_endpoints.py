...
async def test_endpoint(
    client: httpx.AsyncClient, name: str, path: str, params: dict = None
):
    url = f"{BASE_URL}{path}"
...
async def run_all_tests():
    ...
    async with httpx.AsyncClient() as client:
        ...
        await test_endpoint(
            client, "Track Playback", "/track/", {"id": SAMPLE_TRACK_ID}
        )
        ...
        # 8. Album Similar
        await test_endpoint(
            client, "Similar Albums", "/album/similar/", {"id": SAMPLE_ALBUM_ID}
        )

        # 9. Artist Details (Basic)
        await test_endpoint(
            client, "Artist Details", "/artist/", {"id": SAMPLE_ARTIST_ID}
        )

        # 10. Artist Albums & Tracks
        await test_endpoint(
            client, "Artist Albums/Tracks", "/artist/", {"f": SAMPLE_ARTIST_ID}
        )

        # 11. Artist Similar
        await test_endpoint(
            client, "Similar Artists", "/artist/similar/", {"id": SAMPLE_ARTIST_ID}
        )
        ...
        # 13. Cover by Query
        await test_endpoint(
            client, "Cover by Query", "/cover/", {"q": "discovery daft punk"}
        )
        ...
        # 16. Playlist
        await test_endpoint(
            client, "Playlist", "/playlist/", {"id": SAMPLE_PLAYLIST_ID}
        )
